...
public final class MovementPredictionTracker {
    ...
    public void record(final long tick, final Position3f position, final boolean onGround,
        ...
        this.snapshots.put(tick, new Snapshot(position, onGround, horizontalCollision));
    ...
    public Correction replay(final long tick, final Position3f authoritativePosition, final boolean authoritativeOnGround,
        ...
        this.snapshots.tailMap(tick, true).replaceAll((snapshotTick, snapshot) ->
            new Snapshot(snapshot.position().add(correctionDelta), snapshot.onGround(), snapshot.horizontalCollision())
        );
        ...
        // an oscillating backward/forward loop while a movement key is held. Settle X/Z directly
        // to the authoritative point while either end of the replay is horizontally colliding,
        // but keep replaying Y so a later jump or step is not discarded.
        final boolean stabilizeHorizontal = correctedSnapshot.horizontalCollision() || currentHorizontalCollision;
        ...
        final Position3f replayedPosition = new Position3f(
            stabilizeHorizontal ? authoritativePosition.x() : currentPosition.x() + correctionDelta.x(),
            stabilizeVertical ? currentPosition.y() : currentPosition.y() + correctionDelta.y(),
            stabilizeHorizontal ? authoritativePosition.z() : currentPosition.z() + correctionDelta.z()
        );
